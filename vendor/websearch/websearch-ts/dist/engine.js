...
function domainOk(url, inc, exc) {
    ...
    if (inc?.length && !inc.some((d) => host === d.toLowerCase() || host.endsWith("." + d.toLowerCase())))
        return false;
    if (exc?.length && exc.some((d) => host === d.toLowerCase() || host.endsWith("." + d.toLowerCase())))
        return false;
    return true;
}
async function retrieve(query, opts) {
    ...
    if (canPay) {
        ...
        if (paid.length) {
            await budget.recordPaid(config.serpCostPerQuery);
            return paid;
        }
    }
...
export async function search(query, opts = {}) {
    ...
    const hit = await cache.cacheGet(cacheKey);
    if (hit)
        return hit;
    ...
    for (const c of candidates) {
        if (!c.url || seen.has(c.url))
            continue;
        if (!domainOk(c.url, opts.includeDomains, opts.excludeDomains))
            continue;
        seen.add(c.url);
        filtered.push(c);
    }
    ...
    if (deep || needRaw || opts.includeAnswer)
        extracted = await fetchAndExtractMany(top.map((c) => c.url));
    ...
    if (opts.includeAnswer)
        answer = await generateAnswer(query, (answerCtx.length ? answerCtx : results.map((r) => r.content)).slice(0, 6), false);
    const resp = { query, answer, results, responseTime: +((Date.now() - t0) / 1000).toFixed(3) };

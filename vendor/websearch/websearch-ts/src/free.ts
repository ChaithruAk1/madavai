...
async function searxng(query: string, opts: SearchOptions): Promise<Candidate[]> {
  ...
  try {
    ...
    return (data.results ?? []).map((i: any) => ({ title: i.title ?? "", url: i.url ?? "", content: i.content ?? "" })).filter((c: Candidate) => c.url);
  } catch { return []; } finally { clearTimeout(t); }
}

// DuckDuckGo HTML endpoint (no key, no infra; best-effort, can be rate-limited).
async function duckduckgo(query: string): Promise<Candidate[]> {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), config.fetchTimeoutMs);
  let html: string;
  try {
...
export function parseDuckduckgo(html: string): Candidate[] {
...
export async function freeSearch(query: string, opts: SearchOptions): Promise<Candidate[]> {

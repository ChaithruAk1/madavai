...
export function chunkText(text: string, maxChars = 500): string[] {
  ...
  for (let p of parts) {
    ...
    else { if (cur) chunks.push(cur); if (p.length <= maxChars) cur = p; else { for (let i = 0; i < p.length; i += maxChars) chunks.push(p.slice(i, i + maxChars)); cur = ""; } }
  }
...
async function embeddingRerank(query: string, texts: string[]): Promise<number[]> {
  ...
  const emb: number[][] = d.data.map((x: any) => x.embedding);
  const q = emb[0];
  return texts.map((_, i) => cosine(q, emb[i + 1]));
}

// Rerank via Cohere Rerank API (best-quality cross-encoder; optional).
async function cohereRerank(query: string, texts: string[]): Promise<number[]> {
  ...
  const d = await r.json() as any;
  const scores = new Array(texts.length).fill(0) as number[];

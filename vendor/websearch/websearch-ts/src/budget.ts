...
function rollMem(): void { const m = monthKey(); if (mem.month !== m) { mem.month = m; mem.spent = 0; mem.paid = 0; mem.free = 0; } }
...
let warnedMonth = "";
function maybeWarn(spent: number): void {
  const m = monthKey();
...
export async function currentSpend(): Promise<number> {
  const r = await getRedis();
  if (r) return Number((await r.get(`ws:spend:${monthKey()}`)) ?? 0);
  rollMem(); return mem.spent;
}
...
export async function recordPaid(cost: number): Promise<void> {
  const r = await getRedis();
  if (r) {
  ...
  rollMem(); mem.spent += cost; mem.paid++; maybeWarn(mem.spent);
}

export async function recordFree(): Promise<void> {
  const r = await getRedis();
  if (r) { await r.incr(`ws:free:${monthKey()}`); return; }
  rollMem(); mem.free++;
}

export async function usage(): Promise<Usage & { mode: BudgetMode; overBudget: boolean }> {
  const m = monthKey();
  const spent = await currentSpend();
  let paid = mem.paid, free = mem.free;
  const r = await getRedis();
  if (r) { paid = Number((await r.get(`ws:paid:${m}`)) ?? 0); free = Number((await r.get(`ws:free:${m}`)) ?? 0); }
  return {
    month: m,
...
export function setBudget(usd: number): void { config.monthlyBudgetUsd = usd; }
export function setBudgetMode(mode: BudgetMode): void { config.budgetMode = mode; }

// Thinkflux for Chrome — side panel (agent brain) with multi-provider model picker.
...
async function askLLM(messages) {
  ...
  try {
    res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...(c.apiKey ? { Authorization: "Bearer " + c.apiKey } : {}), "HTTP-Referer": "https://thinkflux.local", "X-Title": "Thinkflux for Chrome" },
...
const SYSTEM = `You are Thinkflux, a browsing agent controlling ONE Chrome tab.

...
export function modelFit(modelId, caps = {}, task = {}) {
  ...
  // Capable = clears the office/deck capability gate AND is not flagged non-tool-calling.
  const capable = isDeckCapable(modelId) && caps.agentic !== false;
  if (!heavy) return { fit: "good", label: "Good", why: "Handles chat and everyday tasks." };
  if (capable) return { fit: "good", label: "Recommended", why: "Strong at multi-step, tool-using work." };

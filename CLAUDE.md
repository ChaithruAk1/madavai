...
## SINGLE-SOURCE FIX RULE — Every fix must move toward ONE shared source. No temporary or per-surface band-aids.
Before fixing anything, Claude asks where the logic belongs in the single-source architecture and fixes it THERE, so both web and desktop inherit it from one place. The shared renderer (`src/**`) and shared core (`core/**`) are the default home for any cross-platform logic. Claude must NOT paper over a symptom with a throwaway patch, and must NOT deepen web↔desktop divergence by fixing only one surface's private copy (`electron/*.cjs`, `src/bridge/webBridge.js`) when the real home is shared. If the properly-aligned fix is large, Claude says so in plain words, recommends it, and only uses a stopgap when the owner explicitly approves it — labelled as interim, with the real single-source fix written into PENDING/roadmap so it is never forgotten. The ONLY exception is platform plumbing that genuinely must differ (terminal, local models, file/secret storage). (Owner preference, recorded 2026-06-18.)

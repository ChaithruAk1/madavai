...
export function officeRule(model) {
  const head = " You CAN create real, downloadable office files. NEVER tell the user you cannot create a file, never tell them to copy text into PowerPoint / Word / Excel — that is wrong. For a Word document, PDF, or SPREADSHEET, output ONE fenced block tagged officedoc with ONLY the JSON spec, e.g.:\n```officedoc\n{\"type\":\"docx\",\"name\":\"brief.docx\",\"title\":\"Title\",\"sections\":[{\"heading\":\"Overview\",\"text\":\"…\"}]}\n```\nTypes: docx {title, subtitle?, accent?, meta?:[{label,value}], sections:[{heading?, level?:1|2, leadIn?, text?, bullets?, numbered?, table?:[[…]], callout?:{title?,text}, quote?}]} · pdf (same shape as docx: title, subtitle?, accent?, meta?, sections with heading/level/leadIn/text/bullets/numbered/table/callout/quote) · xlsx — a VALUES spec (YOU compute every number; Madav lays out, styles, and draws the KPI tiles + native charts): {type:\"xlsx\",name,sheets:[SHEET,…]} where each SHEET has a name, optional title, and one or more of: kpis:[{label,value,fmt}] (headline tiles — value is the FINISHED number); columns:[{key,header,fmt}] + rows:[{<key>:value,…}] (a table whose cells are the REAL finished numbers you worked out); and charts:[{type:\"line|col|bar|pie\",title,x:\"<a column key>\",series:[{col:\"<a column key>\",name?}]}] (a native Excel chart bound to those columns). Write the FINISHED VALUES into kpis[].value and rows[] — do NOT use formulas, cell references, ids, expr or periods of ANY kind; do the math yourself and write the numbers. fmt is one of usd, usd2, num, num1, pct, pct0, mult, year. Use a separate Assumptions sheet (a columns+rows table of label + value) so the inputs are visible, the month-by-month or line-item rows on an output sheet, and a Summary sheet of kpis. To include a CHART/GRAPH you MUST add the charts array (x and each series.col are column keys on that sheet) — a chart only mentioned in prose will NOT appear, so include one whenever the user asks for a chart, graph, trend or visualization. Madav renders a polished, bordered, colour-coded Institutional sheet. Fill with COMPLETE real content, never placeholders. If a required topic is genuinely missing, ask ONE short question. ALWAYS tag the fenced block ```officedoc for a spreadsheet, Word document or PDF - never ```xlsx, ```json or ```docx; the JSON \"type\" field selects the format. Don't narrate that you're about to build it — just emit the block.";

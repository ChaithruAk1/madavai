...
import { renderTemplatePreview, sheetTabChrome } from "./doc/xlsxTemplatePreview.js";
...
// Weak models sometimes emit JSON with a trailing comma, a mismatched closer ({ … ]) or a truncated tail.
// Repair those minimal slips so a near-miss still becomes a file instead of a confusing raw snippet. Strict
// parse is tried first, so well-formed specs are never touched.
function _lenientParse(text) {
  const s = String(text == null ? "" : text);
  try { return JSON.parse(s); } catch {}
  const noTrail = s.replace(/,\s*([}\]])/g, "$1");
  try { return JSON.parse(noTrail); } catch {}
  const out = []; const stack = []; let inStr = false, esc = false;        // match each closer to its opener; close truncated tails
  for (const ch of noTrail) {
    if (inStr) { out.push(ch); if (esc) esc = false; else if (ch === "\\") esc = true; else if (ch === '"') inStr = false; continue; }
    if (ch === '"') { inStr = true; out.push(ch); continue; }
    if (ch === "{" || ch === "[") { stack.push(ch); out.push(ch); continue; }
    if (ch === "}" || ch === "]") { const o = stack.pop(); if (!o) continue; out.push(o === "[" ? "]" : "}"); continue; }
    out.push(ch);
  }
  while (stack.length) { const o = stack.pop(); out.push(o === "[" ? "]" : "}"); }
  try { return JSON.parse(out.join("").replace(/,\s*([}\]])/g, "$1")); } catch { return null; }
}

export function parseOfficeSpec(jsonText, opts) {
  try {
    const text = String(jsonText || "");
    // While a spec is still STREAMING (opts.lenient === false) parse STRICTLY — a partial chunk fails so the card
    // keeps a quiet "Preparing…" placeholder; only the FINAL spec gets the lenient repair. This stops the preview
    // flickering a half-built spec on every token (and the O(n) repair from running per token).
    let spec;
    if (opts && opts.lenient === false) { try { spec = JSON.parse(text); } catch { return null; } }
    else spec = _lenientParse(text);
    if (!spec || typeof spec !== "object") return null;
    if (!["xlsx", "docx", "pptx", "pdf"].includes(spec.type)) {
      // Weak models often drop or mis-name the "type" field. Infer it from the shape so the file still
      // builds (a spec with sheets is a spreadsheet, slides a deck, sections a document) instead of a snippet.
      if (Array.isArray(spec.sheets)) spec.type = "xlsx";
      else if (Array.isArray(spec.slides)) spec.type = "pptx";
      else if (Array.isArray(spec.sections)) spec.type = "docx";
      else return null;
    }
...
async function buildXlsxTemplate(spec) {
  ...
  const wantedCharts = (Array.isArray(spec.sheets) ? spec.sheets : []).reduce((n, sh) => n + ((Array.isArray(sh.charts) && sh.charts.length) || 0), 0);
  if (wantedCharts && (charts || []).length < wantedCharts) console.warn(`[office] only ${(charts || []).length}/${wantedCharts} chart(s) resolved \u2014 the rest were dropped (unresolved series/categories).`);
  let buf = await wb.xlsx.writeBuffer();
  try { const ch = await import("./doc/xlsxChart.js"); buf = await ch.injectCharts(buf, charts || []); } catch (e) { console.warn("[office] post-process failed:", (e && e.message) || e); } // ALWAYS: forces recalc-on-open so formula cells/charts aren't blank, and injects any native charts
...
function _previewXlsx(spec) {
  ...
  const blocks = sheets.slice(0, 12).map((sh, i) => {
    ...
    return `<div class="sheet shpane" id="shs${i}"><div class="sname">${_eh(sh.name || "Sheet")}</div><div class="scroll"><table>${body || "<tr><td>(empty)</td></tr>"}</table></div></div>`;
  }).join("");
  ...
  const tc = sheetTabChrome(sheets.slice(0, 12).map((sh) => String(sh.name || "Sheet")));
  return `<!doctype html><html><head><meta charset="utf-8"><style>${css}${tc.css}</style></head><body>${tc.radios}${tc.bar}<div class="sheets">${blocks}</div></body></html>`;
}
...
export function isDeckCapable(model) {
  ...
  const _moe = m.match(/a(\d+(?:\.\d+)?)b\b/); if (_moe && parseFloat(_moe[1]) < 20) return false; // MoE: judge by ACTIVE params ("...-a12b" computes like a small model)
  if (/(nano|mini|small|flash|haiku|lite|tiny|phi-|gpt-oss|gemma-2-2b|\b[1-9]b\b|3b\b|7b\b|8b\b|9b\b)/.test(m)) return false; // weak → reliable template
...
export function officeRule(model) {
  const head = " You CAN create real, downloadable office files. NEVER tell the user you cannot create a file, never tell them to copy text into PowerPoint / Word / Excel — that is wrong. For a Word document, PDF, or SPREADSHEET, output ONE fenced block tagged officedoc with ONLY the JSON spec, e.g.:\n```officedoc\n{\"type\":\"docx\",\"name\":\"brief.docx\",\"title\":\"Title\",\"sections\":[{\"heading\":\"Overview\",\"text\":\"…\"}]}\n```\nTypes: docx {title, subtitle?, accent?, meta?:[{label,value}], sections:[{heading?, level?:1|2, leadIn?, text?, bullets?, numbered?, table?:[[…]], callout?:{title?,text}, quote?}]} · pdf (same shape as docx: title, subtitle?, accent?, meta?, sections with heading/level/leadIn/text/bullets/numbered/table/callout/quote) · xlsx — a FORMULA-DRIVEN model spec: you write the RELATIONSHIPS and Madav writes every real Excel formula, so EXCEL computes the numbers (always internally consistent, and the model recalculates when an assumption changes) and Madav can NEVER write a broken/corrupting formula. Output is small, so do NOT compute or hand-type the period numbers yourself. {type:\"xlsx\",name,sheets:[SHEET,…]} where each SHEET has a name, optional title, and one or more of: inputs:[{id,label,value,fmt,note?}] — your ASSUMPTIONS, the ONLY hardcoded numbers; each needs an id so formulas can reference it; rendered as a clean per-row label/value list, each value in its OWN fmt (so 3.5% and $25,000 show right). metrics:[{id,label,fmt,total?,expr,firstExpr?}] WITH periods:{count,label:\"M%d\"} — a projection where each metric is a ROW and the periods are COLUMNS; expr is the formula using [id] references (e.g. \"[starting_mrr]+[new_mrr]-[churn_mrr]\"), firstExpr is the period-1 formula, [id@-1] is the previous period, total:true makes a bold subtotal. kpis:[{label,ref,fmt}] — headline tiles. ref is EITHER a [Sheet!metric#period] pointing to a metric you ACTUALLY DEFINED on that sheet, OR (for a derived ratio like LTV, CAC payback, LTV:CAC) an inline formula over [Assumptions!...] inputs, which always exist. NEVER reference a metric id you did not define — it resolves to 0 — and NEVER put a bare label here. charts:[{type:\"line|col|bar|pie\",title,x:\"periods\",series:[{metric:\"<a metric id>\",name?}]}] — declare charts on the projection sheet; Madav collects them onto a Charts tab. EXPRESSIONS use [id] in square brackets, NEVER A1 addresses: [id]=that id this period, [id@-1]=previous period, [Sheet!id]=an input/metric on another sheet, [Sheet!metric#p]=that metric at period p, [Sheet!metric#a:b]=a range (wrap in SUM()). Reference inputs and metrics ONLY by their id — never hardcode a number inside a formula. fmt: usd, usd2, num, num1, pct, pct0, mult, year. WORKED PATTERN to copy: Assumptions.inputs = starting_mrr, new_mrr, monthly_churn, arpu, cac, gross_margin; Projection.metrics over 12 periods = starting_mrr (firstExpr [Assumptions!starting_mrr], else [ending_mrr@-1]), new_mrr ([Assumptions!new_mrr]), churn_mrr ([starting_mrr]*[Assumptions!monthly_churn]), ending_mrr ([starting_mrr]+[new_mrr]-[churn_mrr]); Summary.kpis = Ending MRR [Projection!ending_mrr#12], LTV [Assumptions!arpu]*[Assumptions!gross_margin]/[Assumptions!monthly_churn], CAC Payback [Assumptions!cac]/([Assumptions!arpu]*[Assumptions!gross_margin]), LTV:CAC ([Assumptions!arpu]*[Assumptions!gross_margin]/[Assumptions!monthly_churn])/[Assumptions!cac] — the ratios are computed straight from inputs, NEVER pulled from Projection rows you didn't define; a chart of ending_mrr over periods declared on the Projection sheet. Fill with COMPLETE real assumptions, never placeholders. Madav renders a polished, bordered, colour-coded Institutional sheet with native charts. Fill with COMPLETE real content, never placeholders. If a required topic is genuinely missing, ask ONE short question. ALWAYS tag the fenced block ```officedoc for a spreadsheet, Word document or PDF - never ```xlsx, ```json or ```docx; the JSON \"type\" field selects the format. Don't narrate that you're about to build it — just emit the block.";

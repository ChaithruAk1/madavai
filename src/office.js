...
export function officeRule(model) {
  ...
  const weakRule = " You CAN create real, downloadable files — Word, PDF, Excel and PowerPoint. NEVER say you can't, and NEVER tell the user to copy text into an app. To create one, reply with ONE fenced ```officedoc block containing ONLY a JSON spec and NOTHING ELSE — no preamble, no reasoning, no thinking out loud, no text before or after it. SPREADSHEET (Excel): {\"type\":\"xlsx\",\"name\":\"data.xlsx\",\"sheets\":[{\"name\":\"Sheet1\",\"rows\":[[\"Region\",\"Sales\"],[\"North\",1200],[\"South\",900]]}]} — a header row then rows of the REAL finished numbers you work out yourself; do NOT use formulas, cell references, or separate assumptions/periods sheets. WORD: {\"type\":\"docx\",\"name\":\"brief.docx\",\"title\":\"Title\",\"sections\":[{\"heading\":\"Overview\",\"text\":\"…\"}]}. PDF: the same shape with \"type\":\"pdf\". SLIDES: {\"type\":\"pptx\",\"name\":\"deck.pptx\",\"title\":\"…\",\"slides\":[{\"layout\":\"bullets\",\"title\":\"…\",\"bullets\":[\"…\"]}]}. Use real, COMPLETE content, never placeholders. Output the officedoc block immediately and then stop.";
  if (!isDeckCapable(model)) return weakRule;
  return head + deckBespoke + xlsxSimpleNote + tail;
}

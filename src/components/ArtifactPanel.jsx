...
import { X, Eye, Code as CodeIcon, ExternalLink, Copy, Download, RotateCw, Check, Pencil, Sparkles, Undo2, Loader2, Bookmark, Share2, MoreHorizontal } from "lucide-react";
...
export default function ArtifactPanel({ artifact: artifactProp, versions = [], onClose, onResize }) {
  ...
  const isOffice = artifact.kind === "office"; // spreadsheets/docs get a minimal, Claude-style header
  const pickVersion = (i) => { setVIdx(i); setDraft(null); setEditHistory([]); setAiErr(""); };

  // Esc closes the panel (Claude-like) — but never while editing, so unsaved canvas edits aren't lost.
  useEffect(() => {
    const onKey = (e) => {
  ...
  return (
    <div className="artifact-wrap">
      <div className="artifact-resize" onMouseDown={onResize} title="Drag to resize" />
      <div className="artifact-head">
        ...
        <div className="artifact-actions">
          {!isOffice && editHistory.length > 0 && <button className="artifact-ico" title="Undo last AI revision" onClick={undo}><Undo2 size={14} /></button>}
          ...
          {!isOffice && artifact.previewable && <button className="artifact-ico" title="Open in new tab" onClick={openTab}><ExternalLink size={14} /></button>}
          {!isOffice && <button className="artifact-ico" title="Copy" onClick={copy}>{copied ? <Check size={14} /> : <Copy size={14} />}</button>}
          <button className="artifact-ico" title="Download" onClick={download}><Download size={14} /></button>
          {!isOffice && <span className="artifact-roomwrap">
            <button className="artifact-ico" title="More" onClick={openRoomMenu}><MoreHorizontal size={15} /></button>
            {roomMenu && (
              <div className="artifact-roommenu">
                <button className="artifact-roommenu-row" onClick={() => { setRoomMenu(false); saveToGallery(); }}>
                  <span className="artifact-roommenu-glyph">{saved ? <Check size={14} /> : <Bookmark size={14} />}</span> Save to Studio gallery
                </button>

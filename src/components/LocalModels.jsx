        ...
        {(() => {
          ...
          if (showSearch) {
            ...
            if (onlyInstalled) {
              ...
              const curated = galAll.filter(galleryFilter).filter((e) => { if (!selCaps.length) return true; const f = fitForRam(e.sizeGB, sys.totalRamGB); return f === "good" || f === "unknown"; }).filter((e) => !installedBases.has(baseOf(e.pullName))).slice(0, 100).map((e) => ({ name: e.name, pullName: e.pullName, installed: false, sizeGB: e.sizeGB, sizeLabel: e.sizeLabel, downloads: e.downloads, description: e.description, useCases: e.useCases }));
          ...
          if (sortKey) {
            ...
            const val = (r) => sortKey === "model" ? prettyLocalName(r.name || r.pullName).toLowerCase() : sortKey === "size" ? (r.sizeGB || 0) : sortKey === "context" ? ctxNum(r.name || r.pullName) : sortKey === "params" ? paramNum(r.name || r.pullName) : sortKey === "ram" ? fitRank(r) : sortKey === "downloads" ? (r.downloads || 0) : sortKey === "caps" ? capsList(r, isMedia).join(",").toLowerCase() : sortKey === "cost" ? 0 : 0;

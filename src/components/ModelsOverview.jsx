          ...
          <tbody>
            {rows.map((m) => (
              ...
              <tr className={`${isActive(m) ? "active" : ""} ${expanded === m.name ? "expanded" : ""}`} onClick={() => setExpanded(expanded === m.name ? null : m.name)} style={{ cursor: "pointer" }}>
                ...
                <td><div className="mo-caps">{CAP_CHIPS.filter((c) => c.test(m)).map((c) => { const I = c.icon; return <span key={c.key} className="mo-cap" style={{ color: "var(--text-1)", borderColor: "var(--line)" }}><I size={12} style={{ color: c.tone }} /> {c.label}</span>; })}{!CAP_CHIPS.some((c) => c.test(m)) && <span className="mo-cap-none">General</span>}</div></td>

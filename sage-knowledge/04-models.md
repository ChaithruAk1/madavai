...
Behavior: Madav Starter (free) works the moment you sign in (no key, free models, daily limit). Every other default provider needs your OWN API key to activate — sign up on the provider's site (OpenRouter at openrouter.ai is easiest, OpenAI at platform.openai.com, Anthropic at console.anthropic.com), create a key, then paste it into the card so it flips to “Connected”. Local providers (Ollama, LM Studio, llama.cpp) need no key. The highlighted “Custom provider” card sits FIRST and is a template: clicking “Set up” creates a brand-new provider you can name and point at any OpenAI- or Anthropic-compatible endpoint — the template card itself never changes. Default providers cannot be deleted; only providers you create via Custom provider can be removed.  
...
Behavior: Edits save instantly as you type; the API key field shows “leave blank for local” for local endpoints. The trash (delete) button appears ONLY for user‑created Custom providers — default providers are locked and cannot be deleted. A note reminds you that providers stay available simultaneously; the top‑bar model selector decides which one runs. (For Anthropic, admins/creators also get a “My Claude subscription” authentication option that uses the logged‑in Claude CLI session instead of an API key.)  

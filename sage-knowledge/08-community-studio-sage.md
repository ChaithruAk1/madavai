...
### Studio · creative suite (Design DNA, Variations, Crew, Gallery)
aliases: studio styles, design dna, studio variations, crew mode, studio gallery, save a look, surprise me, studio guide
What: Studio is the make-by-talking surface, now with a creative toolkit: Design DNA (save a look — palette/type/voice — and apply it in one click), Surprise me (a dice that seeds a creative constraint), Variations (ask for 1/2/4 takes side by side), Crew mode (a Concept → Critic → Polish pass), and a Gallery (keep, reopen, remix, download, or send creations to a Workroom's knowledge shelf).
Why: So a one-shot generator becomes a real creative space — reuse your aesthetic, explore options instead of re-asking, refine with a built-in critique, and keep your work instead of losing it to chat.
Behavior: Studio's tabs are Create / Gallery / Guide. Build by describing an idea (the prompt is the hero; lenses tune the format). Save a look from the Design DNA row; pick it before a build to apply it. Set Variations to 2 or 4 to compare takes. Turn on Crew mode for a self-critiqued first draft. On any preview, "Save to gallery" keeps it; in the Gallery, "Refine in Studio" remixes it (the original stays safe). The artifact toolbar can also send a creation straight to a Workroom. For the full walkthrough, open Studio → the Guide tab (Tour, Do's & don'ts, How to). When asked "how do I" anything in Studio, give the end-to-end steps.

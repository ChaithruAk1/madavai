...
import { createLocalAiRuntime } from './providers/localai.js';
...
export function createRuntimes(opts: RegistryOptions = {}): Partial<Record<RuntimeId, LocalModelRuntime>> {
  ...
  const localai = createLocalAiRuntime(opts.localaiBase);
  const reg: Partial<Record<RuntimeId, LocalModelRuntime>> = { ollama, huggingface, localai };

...
### Gates
- This session touched **main.cjs / preload.cjs / settings.cjs / agent-transport.cjs / providers.cjs / agent-openai.cjs / session-manager.cjs / mission-runner.cjs (+ NEW harness.cjs, model-stats.cjs)** (main process) + webBridge/mockBridge/package.json/APP-GUIDE/AGENT-GUIDE/RESEARCH/PLAN docs + **SageDock.jsx/Portrait.jsx/Agents.jsx/ModelsOverview.jsx/styles.css (+ NEW src/shared/harness.js, HARNESS.md)** (renderer) → `npm run build` + FULL close-and-reopen, then commit from the user's terminal. Harness batch built AFTER the user's clean commit — `git diff` shows exactly this work. Nothing compile-checked (sandbox down).

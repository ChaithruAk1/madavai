# Proposal: Simplified Career-Ops Module for Madav
...
## 2. Why it's so complex — and why most of it disappears inside Madav
...
| Career-ops component | Why it exists | Madav already provides |
|---|---|---|
...
| `followup-cadence.mjs`, cron-style scripts | No scheduler | Madav scheduled tasks |
...
**Verdict: yes — this can be redesigned far simpler.** Roughly 80% of the repo is infrastructure Madav makes redundant. The durable value is concentrated in: the A–F evaluation methodology (prompt content), the 3 ATS provider clients (~4 small .mjs files), the CV HTML template, and the canonical status model (`states.yml`).

## 3. Proposed design: "Career" module in Madav
...
- Profile/CV live as project knowledge files (`cv.md`, `profile.yml` equivalent) inside a dedicated Madav Project — no new storage concept.
...
2. **scan** (absorbs scan + pipeline + batch): pull new offers from configured portals via providers, dedupe against the store, optionally auto-evaluate top matches. Parallelism handled by Madav's agent runtime, not shell scripts.
...
Two optional scheduled tasks using Madav's existing scheduler: daily portal scan, and daily follow-up check that flags overdue applications.

## 4. What is explicitly cut

Go dashboard and toolchain; `batch-runner.sh` and all batch state files; 5 tracker-integrity scripts; TSV/markdown tracker format; 7 of 8 language trees; LaTeX pipeline; Puppeteer; Nix/release tooling; multi-CLI skill triplication. None of these carry functionality the Madav-native design loses — they carry portability and file-format repair work the design makes unnecessary.
...
1. **Live form-filling (apply mode)** depends on browser automation; in Madav this should route through an MCP browser connector rather than bundling Playwright. Defer to Phase 4.
...
4. Comp research quality depends on web search availability in the agent runtime — already present in Madav.

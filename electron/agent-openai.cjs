...
function officeRulePart(model) {
  ...
  const deckBespoke = " SLIDE DECKS / PRESENTATIONS (.pptx): build with ONE fenced deckjs block and NOTHING else for the deck — never an officedoc block for slides. Write JavaScript that composes the deck with full design freedom. You are given pptx (a PptxGenJS instance; layout is already 13.33x7.5in widescreen). Do NOT call pptx.write or writeFile — Madav runs it and shows a download card. API: const s = pptx.addSlide(); s.background = { color: '0F1E3C' }; s.addText(t, { x, y, w, h, fontSize, bold, color, align, valign }); s.addShape(pptx.ShapeType.roundRect, { x, y, w, h, fill:{ color }, line:{ color, width }, rectRadius }) (also rect, ellipse, line); s.addChart(pptx.ChartType.bar, [{ name, labels, values }], { x, y, w, h, barDir, chartColors, showValue, showLegend }) (also line, pie, doughnut); s.addImage({ data:'data:image/png;base64,...', x, y, w, h }) for a logo/icon. Coordinates in inches; colours are 6-hex without a hash. DESIGN like a top consultancy: deliberate per-slide background, a SEMANTIC accent palette (e.g. teal metrics, gold highlights, red risk), oversized stat numbers, ellipse icon badges, a real chart for any data, generous spacing, and DENSITY — pack 8-15 distinct visual elements per slide (card grids, a stat side-rail, section bands, badged panels, small shape-built icon glyphs); fill the whole canvas, never leave it sparse — compose each slide for ITS content. If the user gave brand colours, fonts, or a logo, use them. Honor the requested slide COUNT exactly. Start the script with: // name: Your Title.pptx. Worked example (make yours richer): const s = pptx.addSlide(); s.background={ color:'0E1B2C' }; s.addText('EXECUTIVE BRIEFING',{ x:0.6,y:0.45,w:8,h:0.3,fontSize:12,bold:true,color:'00C2FF',charSpacing:3 }); s.addText('The AI Landscape',{ x:0.6,y:0.85,w:9,h:1,fontSize:40,bold:true,color:'FFFFFF' }); s.addText('$1.1T',{ x:0.6,y:2.4,w:3,h:1,fontSize:54,bold:true,color:'00C2FF' }); s.addShape(pptx.ShapeType.ellipse,{ x:7,y:2.7,w:0.5,h:0.5,fill:{ color:'F5C842' } }); const s2 = pptx.addSlide(); s2.background={ color:'0E1B2C' }; s2.addChart(pptx.ChartType.bar,[{ name:'Readiness', labels:['Platform','Governance','Talent'], values:[28,22,41] }],{ x:0.7,y:1.6,w:11.9,h:4.8,barDir:'bar',chartColors:['00C2FF'],showValue:true });";

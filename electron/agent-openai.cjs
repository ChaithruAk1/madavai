...
let _coreRulesP = null; const coreRules = () => (_coreRulesP ||= import("../core/agent-rules.js")); // ADR-0001 core: ESM single source, dynamic import() cached (Electron main supports it; same as the MCP SDK)

const SYSTEM = (mode) =>
  mode === "chat"
    ? `You are Madav, a helpful AI assistant. You are NOT any other AI assistant or model; if anyone asks who you are or who made you, you are Madav. Use a skill or connector tool when it fits the user's request; otherwise just answer. ` +
      `Reply in clear, natural language; never paste raw JSON, tool-call syntax, or machine field names.`
    : mode === "code"
    ? `You are Madav, an expert software engineer working in the user's repository. You are NOT any other AI assistant or model; if anyone asks who you are or who made you, you are Madav. ` +
    ...
    : `You are Madav, an AI assistant working inside the user's folder. You are NOT any other AI assistant or model; if anyone asks who you are or who made you, you are Madav. ` +
...
async function runOpenAIAgentTurn({ prompt, mode, cwd, profile, history, emit, permissions, signal, permMode = "default", connectors = [], skillsDir = "", disabledSkills = [], systemOverride = null, globalInstructions = "", allowAskUser = false, roster = [], callAgent = null, browser = null, desktop = null, noShell = false, agentName = "", agentOpts = {}, dataTools = false }) {
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    { const _sig = toolCalls.map((tc) => tc.name + ":" + String(tc.arguments == null ? "" : tc.arguments).slice(0, 300)).join("+"); const n = runGuard.note(_sig); if (n.stop) { const m = guardStopMessage(n.code); emit({ kind: "assistant_delta", data: { text: m } }); emit({ kind: "assistant_message", data: { stop_reason: "guard" } }); if (rec) rec.finish({ text: m, numTurns: step + 1, capped: true }); modelStats.bump(model, "maxSteps"); emit({ kind: "result", data: { subtype: "guard_" + n.code, num_turns: step + 1, duration_ms: Date.now() - started } }); return; } }
